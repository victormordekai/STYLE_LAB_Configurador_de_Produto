...
(() => {
  ...
  function syncClipPathsFromProductModel() {
    ...
    SURFACE_ORDER.forEach((surface) => {
      const geometry = window.StyleLabGarmentModel.geometryFor({ family: state.productFamily });
  ...
  function applyTechnicalModelState() {
    Object.values(technicalModels).forEach((model) => {
      ...
      model.dataset.family = state.productFamily;
    });
  }
  ...
  function restore(data) {
    ...
    state.productFamily = data.productFamily || state.productFamily;
  ...
  function syncConstructionUI() {
    ...
    if (collar) collar.value = state.construction.collar === "v" ? "V" : state.construction.collar === "polo" ? "POLO" : "REDONDA";
  ...
  function patternMarkup(kind, width, height) {
    if (window.StyleLabFormsLibrary?.patternMarkup) {
      return window.StyleLabFormsLibrary.patternMarkup(kind, width, height, state.colors);
    }
    return `<rect width="${width}" height="${height}" fill="${state.colors.detail}"/>`;
  }
  ...
  function elementMarkup(element) {
    ...
    if (element.type === "shield") {
      const symbol = escapeHtml(element.data.symbol || "🛡️");
      if (window.StyleLabFormsLibrary?.shieldMarkup) {
        return window.StyleLabFormsLibrary.shieldMarkup(symbol, element.width, element.height, state.colors);
      }
      return `<text x="${element.width/2}" y="${element.height*.75}" text-anchor="middle" font-size="${Math.min(element.width, element.height)*.75}">${symbol}</text>`;
    }
  ...
  function renderElementInspector(element) {
    ...
    panel.hidden = false;
    $("#inspectorTitle").textContent = `${element.name.toUpperCase()} · CONTROLES`;
    const size = $("#elementSizeRange"), sizeOut = $("#elementSizeValue");
    const fontRow = $("#fontSizeRow"), font = $("#fontSizeRange"), fontOut = $("#fontSizeValue");
    if (size) { size.value = String(Math.round(element.width)); sizeOut.textContent = `${Math.round(element.width)} px`; }
    const textual = ["text","number","sponsor"].includes(element.type);
    if (fontRow) fontRow.hidden = !textual;
    if (textual && font) { const fs=element.data.fontSize || Math.min(element.height*.82,62); font.value=String(Math.round(fs)); fontOut.textContent=`${Math.round(fs)} px`; }
  }

  function bindElementInspector() {
    const size=$("#elementSizeRange"), font=$("#fontSizeRange");
    let sizeBefore=null, fontBefore=null;
    size?.addEventListener("pointerdown",()=>{ if(selected()&&!sizeBefore) sizeBefore=snapshot(); });
    size?.addEventListener("input",(event)=>{
      const el=selected(); if(!el||el.locked)return;
      const next=Number(event.target.value), ratio=el.height/Math.max(el.width,1);
      el.width=next; el.height=Math.max(20,Math.min(520,next*ratio)); renderAll();
    });
    size?.addEventListener("change",()=>{ if(sizeBefore){pushHistory(sizeBefore);sizeBefore=null;} renderAll(); });
    font?.addEventListener("pointerdown",()=>{ if(selected()&&!fontBefore) fontBefore=snapshot(); });
    font?.addEventListener("input",(event)=>{
      const el=selected(); if(!el||!["text","number","sponsor"].includes(el.type)||el.locked)return;
      el.data.fontSize=Number(event.target.value); renderAll();
    });
    font?.addEventListener("change",()=>{ if(fontBefore){pushHistory(fontBefore);fontBefore=null;} renderAll(); });
  }
  ...
  function addShield(shield) {
    const item = shield || window.StyleLabFormsLibrary?.SHIELDS?.[0] || {symbol:"🛡️",name:"ESCUDO"};
    const p = surfacePlacement();
    createElement({
      type: "shield",
      name: item.name || "ESCUDO",
      target: state.activeSurface,
      x: p.x + 75,
      y: p.y + 90,
      width: 120,
      height: 120,
      data: { symbol: item.symbol || "🛡️" }
    });
  }

  function addShape() {
    const p = surfacePlacement();
    createElement({ type:"shape", name:"FORMA", target:state.activeSurface, x:p.x+60,y:p.y+100,width:150,height:75,data:{color:state.colors.detail,radius:12} });
  }
  ...
  function inlineComputedSvgStyles(sourceRoot, copyRoot) {
    const sourceNodes=[sourceRoot,...sourceRoot.querySelectorAll("*")];
    const copyNodes=[copyRoot,...copyRoot.querySelectorAll("*")];
    const props=["fill","fill-opacity","stroke","stroke-width","stroke-opacity","stroke-linecap","stroke-linejoin","opacity","font-family","font-size","font-weight","letter-spacing","text-anchor","display","visibility","filter"];
    sourceNodes.forEach((sourceNode,i)=>{
      const target=copyNodes[i]; if(!target||!(sourceNode instanceof Element))return;
      const cs=getComputedStyle(sourceNode); const style=props.map(prop=>`${prop}:${cs.getPropertyValue(prop)}`).join(";");
      const existing=target.getAttribute("style")||"";
      target.setAttribute("style", `${existing};${style}`);
    });
  }

  function buildExportSvgCopy() {
    ...
    inlineComputedSvgStyles(canvas,copy);
    copy.querySelector("#selectionLayer")?.remove();
    return copy;
  }

  function exportSvg() {
    const copy=buildExportSvgCopy();
  ...
  $("#collarSelect")?.addEventListener("change", (event) => {
    transaction(() => { state.construction.collar = event.target.value === "V" ? "v" : event.target.value === "POLO" ? "polo" : "round"; });
  });
  ...
  function initializeProductPicker() {
    const select=$("#productSelect"); if(!select||!window.StyleLabModelingLibrary)return;
    const molds=window.StyleLabModelingLibrary.listMolds();
    select.innerHTML=molds.map(m=>`<option value="${m.sku}">${escapeHtml(m.label||m.sku)} · ${m.sku}</option>`).join("");
    const current=window.StyleLabModelingLibrary.getMoldForSku(state.productId); if(current)select.value=current.sku;
    const apply=()=>{const mold=window.StyleLabModelingLibrary.getProduct(select.value);if(!mold)return;transaction(()=>{state.productId=mold.sku;state.productFamily=mold.family||"shirt-basic";state.selectedId=null;});mountTechnicalModels();renderAll();};
    select.addEventListener("change",apply);$("#changeProductBtn")?.addEventListener("click",apply);
  }
  ...
  let activeLibraryCategory = "patterns";
  function renderLibraryCategory(category="patterns") {
    activeLibraryCategory=category;
    const grid=$("#patternGrid"), load=$("#loadPatternBtn"); if(!grid)return;
    grid.innerHTML="";
    if(category==="patterns"){
      (window.StyleLabFormsLibrary?.PATTERNS||[]).forEach((item,index)=>{
        const b=document.createElement("button"); b.className=`library-card pattern-card${index===0?" selected":""}`; b.dataset.pattern=item.id; b.innerHTML=(window.StyleLabFormsLibrary.cardSvg(item))+`<small>${escapeHtml(item.name)}</small>`;
        b.addEventListener("click",()=>{grid.querySelectorAll(".library-card").forEach(x=>x.classList.remove("selected"));b.classList.add("selected");addPattern(item.id);});grid.appendChild(b);
      });
      if(load){load.textContent="CARREGAR PADRÃO";load.style.display="";}
    } else if(category==="shields"){
      (window.StyleLabFormsLibrary?.SHIELDS||[]).forEach((item)=>{
        const b=document.createElement("button");b.className="library-card shield-card";b.innerHTML=`<span>${escapeHtml(item.symbol)}</span><small>${escapeHtml(item.name)}</small>`;b.addEventListener("click",()=>addShield(item));grid.appendChild(b);
      });
      if(load)load.style.display="none";
    } else {
      const labels={numbers:"ADICIONAR NÚMERO",names:"ADICIONAR NOME",sponsors:"ADICIONAR PATROCÍNIO"};
      grid.innerHTML=`<div class="element-inspector" style="grid-column:1/-1"><h3>${labels[category]}</h3><div class="inspector-note">Clique abaixo para inserir o elemento e depois selecione-o para ajustar tamanho e fonte dinamicamente.</div></div>`;
      const b=document.createElement("button");b.className="wide-btn";b.style.gridColumn="1/-1";b.textContent=labels[category];b.addEventListener("click",()=>category==="numbers"?addNumber():category==="names"?addText():addSponsor());grid.appendChild(b);
      if(load)load.style.display="none";
    }
  }

  $("#loadPatternBtn")?.addEventListener("click",()=>{ const card=$("#patternGrid .pattern-card.selected"); addPattern(card?.dataset.pattern||"bolt-a"); });
  $$(".category").forEach((button)=>button.addEventListener("click",()=>{ $$(".category").forEach(i=>i.classList.remove("active"));button.classList.add("active");renderLibraryCategory(button.dataset.category); }));
  ...
  renderLibraryCategory("patterns");
